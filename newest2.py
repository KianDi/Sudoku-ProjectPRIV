...
class SudokuGenerator:
    ...
    def is_valid(self, row, col, num):
        return (
                SudokuGenerator.valid_in_row(self, row, num)
                and SudokuGenerator.valid_in_col(self, col, num)
                and SudokuGenerator.valid_in_box(self, row - row % self.box_length, col - col % self.box_length, num)
        )
...
def draw_game_over_lost_screen(screen):
    ...
    text_rect = text.get_rect(center=(screen.get_width() // 2, screen.get_height() // 2))
    screen.blit(text, text_rect)
def draw_game_over_won_screen(screen):
    font = pygame.font.Font(None, 36)
...
def start_sudoku(difficulty, screen):
    ...
    sudoku_board.sudoku_generator = sudoku_generator
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if Board.is_full(sudoku_board):
                ...
                if Board.check_board(sudoku_board) is:
                    print("Win!!!!")
            '''
            if Board.is_full(sudoku_board) == True:
                if Board.check_board(sudoku_board) == True:
                    screen.fill((255, 255, 255))
                    draw_game_over_won_screen(screen)
                    #print("hi")
                    #pygame.display.update()
                    #pygame.display.flip()
                elif Board.check_board(sudoku_board) == False:
                    screen.fill((255, 255, 255))
